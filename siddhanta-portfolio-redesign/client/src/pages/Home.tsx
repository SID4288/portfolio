...
import { ArrowUpRight, ChevronDown, Circle, ExternalLink, Github, Mail, MapPin, MoveUpRight, Radio, Sparkles } from "lucide-react";
...
function SectionLabel({ number, eyebrow, title }: { number: string; eyebrow: string; title: string }) {
  return (
    <div className="section-heading">
      <div className="section-heading__meta"><span>{number}</span><span>{eyebrow}</span></div>
      <h2>{title}</h2>
      ...
      <main id="top">
        <section className="hero container-wide">
          <div className="hero__copy reveal-up">
            <div className="kicker"><Radio size={14} /> COMPUTER ENGINEER · KATHMANDU, NEPAL</div>
            <p className="hero__overline">A field report on building useful intelligence.</p>
            <h1 className="hero__name"><span>Siddhanta</span><em>Adhikari</em></h1>
            <div className="hero__name-meta"><span>IDENTITY / 001</span><i /> computer engineer / researcher</div>
            <h2 className="hero__slogan">Systems <em>with a point</em> of view.</h2>
            <p className="hero__intro">I’m Siddhanta Adhikari — a final-year computer engineering student building machine learning and full-stack systems for messy, real-world problems.</p>
        ...
        <section id="projects" className="content-section container-narrow">
          ...
          <div className="section-intro"><p>Projects that sit at the intersection of perception, computation, and a strong bias toward making things tangible.</p><a href="https://github.com/SID4288?tab=repositories" target="_blank" rel="noreferrer">View repository index <ArrowUpRight size={15} /></a></div>
          <div className="projects-grid">
            {projects.map((project) => <a data-cursor="view" className={project.featured ? "project-card project-card--featured" : "project-card"} href={project.href} target="_blank" rel="noreferrer" key={project.title}>
            ...
            <a data-cursor="view" className="project-card project-card--graveyard" href="https://github.com/SID4288?tab=repositories" target="_blank" rel="noreferrer"><span className="project-card__number">++</span><span className="project-card__stamp">UNRESOLVED</span><div className="project-card__icon">✳</div><h3>~/the-graveyard</h3><p>Where abandoned prototypes, multi-threading benchmarks, and structural refactors go to rest.</p><div className="tag-row"><span>Git stash</span><span>Untracked</span><span>Error: 404</span></div></a>
          </div>
        </section>

        <section id="research" className="research-section">
          <div className="research-section__image"><img src="/images/research.webp" alt="Orbital scan inspired research diagram" /><div className="research-section__image-label">OBSERVATION / 02</div></div>
          <div className="container-narrow research-section__content"><SectionLabel number="02" eyebrow="FIELD NOTES / INQUIRY" title="Research" /><div className="research-list">{research.map((item) => <article className="research-item" key={item.title}><span>{item.status}</span><h3>{item.title}</h3><p>{item.text}</p></article>)}</div></div>
        </section>
        ...
        <section id="certificates" className="certificate-section">
          <div className="container-narrow">
            ...
            <div className="certificate-grid">{certificates.map((certificate) => <article className="certificate-card" key={certificate.title}><span className="certificate-year">{certificate.year}</span><div><h3>{certificate.title}</h3><p>{certificate.issuer}</p><small>{certificate.note}</small></div><ArrowUpRight size={17} /></article>)}</div>
          </div>
        </section>
